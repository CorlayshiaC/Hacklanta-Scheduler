...
| `--on-accent-warn` | `#000000` | same | `text-on-accent-warn` | V4.1. True black, both themes (not text-primary's near-black, which carries a purple cast): the only safe text color on `--accent-warn-pale` or dark theme's `--accent-warn` used directly as a fill. Never pair a champagne fill with `--on-accent` (white), see "Accent fill vs glow" |
...
| `#000000` (on-accent-warn, shipped value, both themes) on `#E9C56A` (warn-pale, light) | 12.66:1 | Pass AAA |
...
**Third follow-up, same day:** further feedback ("no like darker than that") took `#141414` to its
natural endpoint, `#000000`, true black, both themes. 12.93:1 on the dark-theme fill, 12.66:1 on
the light-theme pale fill, both AAA with headroom to spare. No component changes needed for either
follow-up, every consumer reads `--on-accent-warn` through the token.
