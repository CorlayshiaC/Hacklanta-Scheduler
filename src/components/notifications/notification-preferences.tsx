 ...
 *
 * V3: the hand-rolled Radix Switch and the local card shell are gone, replaced by Agent 1's
 * NeuToggle and Card. Nothing in this file styles anything anymore, which is why it picked up the
 * glass surface and the theme switch without an edit.
 */
export function NotificationPreferences() {
  ...
  if (loadState === "loading") {
    return (
      <div aria-label="Loading notification preferences" className="flex flex-col gap-2" role="status">
  ...
  if (loadState === "error") {
    return (
      <Card>
        <p className="text-sm text-accent-warn">Could not load notification preferences.</p>
      </Card>
    );
  }

  return (
    <Card className="divide-y divide-hairline" padded={false}>
      {KINDS.map((kind) => {
        ...
        return (
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              {rowError ? <span className="text-xs text-accent-warn">{rowError}</span> : null}
            </div>
